...
{
    if (adc_unit == ADC_UNIT_2) {
        ESP_RETURN_ON_FALSE(adc2_lock != NULL, ESP_ERR_INVALID_STATE, TAG, "adc2 lock release without acquiring");
        _lock_release(&adc2_lock);
    }

    if (adc_unit == ADC_UNIT_1) {
        ESP_RETURN_ON_FALSE(adc1_lock != NULL, ESP_ERR_INVALID_STATE, TAG, "adc1 lock release without acquiring");
