...
static inline __attribute__((always_inline, pure)) uint32_t xt_utils_get_core_id(void)
{
    /*
...
static inline __attribute__((always_inline, pure)) uint32_t xt_utils_get_raw_core_id(void)
{
#if XCHAL_HAVE_PRID
...
static inline __attribute__((always_inline)) void xt_utils_set_watchpoint(int wp_num, uint32_t wp_addr, size_t size, bool on_read, bool on_write)
{
...
static inline __attribute__((always_inline)) bool xt_utils_dbgr_is_attached(void)
{
    uint32_t dcr = 0;
    uint32_t reg = DSRSET;
    RER(reg, dcr);
...
/*
static inline __attribute__((always_inline)) bool xt_utils_compare_and_set(volatile uint32_t *addr, uint32_t compare_value, uint32_t new_value)
{
